...
interface TransactionRepository<T : Transaction> : PagingAndSortingRepository<T, Long> {
    ...
    @Query("SELECT * from TRANSACTION where src_id = ?1 order by create_Date desc, amount desc, new_value desc, reason desc limit 1", nativeQuery = true)
    ...
    @Query("SELECT * from TRANSACTION where src_id = ?1 and create_Date < ?2 order by create_Date desc, amount desc, new_value desc, reason desc limit 1", nativeQuery = true)
    fun findLastEarlierThan(account: Account, earlierThan: OffsetDateTime): T?

    @Query("SELECT * from TRANSACTION where src_id = ?1 and create_Date >= ?2 order by create_Date, amount desc, new_value desc, reason desc", nativeQuery = true)
